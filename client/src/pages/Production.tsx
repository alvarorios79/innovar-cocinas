...
import {
  ...
  Package, Truck, CheckCircle2, RefreshCw, User,
} from "lucide-react";
...
const PROD_STAGES = [
  ...
  },
  {
    ...
    description: "Corte y mecanizado",
  },
  {
    key: "enchape",
    label: "Enchape",
    ...
    description: "Aplicación de enchape",
  },
  {
    key: "ensamble",
    label: "Ensamble",
    icon: Package,
    color: "#1DB5A8",
    bg: "rgba(29,181,168,0.07)",
    description: "Armado de módulos",
  },
  {
    key: "listo_instalacion",
    label: "Listo",
    icon: CheckCircle2,
    color: "#10B981",
    bg: "rgba(16,185,129,0.07)",
    description: "Listo para instalar",
  },
  {
    key: "entregado",
    label: "Entregado",
    ...
    description: "Instalado y entregado",
  },
] as const;
    ...
    <div className="pb-20 md:pb-6">
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-white/[0.05] text-white/55 border border-white/[0.08]">
          Total en fábrica:
          <strong>{prodProjects.length}</strong>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex gap-4 overflow-x-auto pb-4">
          {PROD_STAGES.map((s) => (
            <div
              key={s.key}
              className="min-w-[260px] h-64 rounded-xl bg-white/[0.06] animate-pulse shrink-0"
            />
          ))}
        </div>
      ) : (
        <div className="flex gap-4 overflow-x-auto pb-4">
          {PROD_STAGES.map((stage) => {
            ...
            return (
              <div
                key={stage.key}
                className="min-w-[260px] max-w-[280px] shrink-0 flex flex-col rounded-xl border"
              ...
              >
                ...
                >
                  ...
                  <Badge
                    className="ml-auto text-[10px] h-5 px-1.5 text-white"
                    ...
                    stageProjects.map((project) => (
                      <ProdCard
                        ...
                        onNext={() => {
                          ...
                          if (ns) move(project.id, ns);
                        }}
                        onPrev={() => {
                          const ps = prevStage(stage.key);
                          if (ps) move(project.id, ps);
                        }}
...
// ── ProdCard ──────────────────────────────────────────────────────────────────
function ProdCard({
  project, stageKey, canAdvance, isPending,
  onNext, onPrev, hasNext, hasPrev,
}: {
  project: Project;
  stageKey: ProdStageKey;
  canAdvance: boolean;
  isPending: boolean;
  onNext: () => void;
  onPrev: () => void;
      ...
      <CardContent className="p-3">
        ...
        <div className="mb-2">
          ...
          <Badge
            ...
            style={{ color: stageInfo.color, borderColor: `${stageInfo.color}40` }}
          >
            {typeLabel}
          </Badge>
        </div>
