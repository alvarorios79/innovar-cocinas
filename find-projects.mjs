...
(async () => {
  try {
    ...
    const [projects] = await connection.execute(`
      SELECT id, name, isArchived, deletedAt, createdAt, userId
      FROM projects
    ...
    if (projects.length === 0) {
      ...
      console.log(`ID | Name | isArchived | deletedAt | CreatedAt | UserId`);
      console.log('-'.repeat(120));
      projects.forEach(p => {
        ...
        console.log(`${p.id} | ${p.name} | ${archivedStatus} | ${deletedStatus} | ${createdDate} | ${p.userId}`);
      });
    }
