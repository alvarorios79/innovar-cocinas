...
// ============ QUOTATION HISTORICAL COPIES ============

export async function createHistoricalCopy(quotationId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  // Contar copias históricas existentes para obtener el siguiente número de versión
  const existingCopies = await db
    .select({ count: sql<number>`COUNT(*)` })
    .from(quotations)
    .where(and(eq(quotations.baseQuotationId, quotationId)));

  const nextVersion = (existingCopies[0]?.count ?? 0) + 1;

  // Crear la copia histórica
  const result = await db.insert(quotations).values({
    ...
    customDescriptions: original.customDescriptions,
    ...
    parentQuotationId: original.parentQuotationId,
    isAdditional: original.isAdditional,
...
export async function getHistoricalCopies(baseQuotationId: number) {
  const db = await getDb();
  if (!db) return [];
  ...
  return await db
    .select()
    .from(quotations)
    .where(and(eq(quotations.baseQuotationId, baseQuotationId), ne(quotations.id, baseQuotationId)))
    .orderBy(desc(quotations.versionNumber));
}

export async function getActiveQuotation(projectId: number) {
  const db = await getDb();
  if (!db) return undefined;

  // Obtener el proyecto para ver su cotización
  const project = await db.select().from(projects).where(eq(projects.id, projectId)).limit(1);
  if (!project || project.length === 0) return undefined;

  const quotationId = project[0].quotationId;
  if (!quotationId) return undefined;

  // Obtener la cotización activa (donde id === baseQuotationId, o sin baseQuotationId)
  const result = await db
    .select()
    .from(quotations)
