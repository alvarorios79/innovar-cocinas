...
// mysql2 removed - using PostgreSQL

let _db: ReturnType<typeof drizzle> | null = null;
// Pool removed - PostgreSQL connection handled by drizzle directly
...
export async function getGlobalFinancialDashboard() {
  ...
  for (const project of allProjects) {
    ...
    const projectExpenses = await db.select().from(expenses).where(and(eq(expenses.projectId, project.id), isNull(expenses.deletedAt)));
  ...
  const operationalExpenses = await db.select().from(expenses).where(and(isNull(expenses.projectId), isNull(expenses.deletedAt)));
...
export async function getCashFlowData() {
  ...
  // Bug 6 fix: filtrar gastos eliminados en el flujo de caja
...
export async function getCEOFinancialMetrics() {
  ...
  try {
    ...
    // Bug 1 fix: se incluyen TODOS los proyectos manuales activos (con o sin quotationId)
    // Bug 2 fix: se usa calculateProjectBalance por proyecto para obtener el total
    //            de la ÚLTIMA versión de cotización (igual que muestra cada proyecto)
    // Bug 4 fix: ingresosRecibidos se suma solo de proyectos dataOrigin='manual'
    // Bug 5 fix: se excluyen proyectos cerrados (accountingClosureId IS NOT NULL)
    // ────────────────────────────────────────────────────────────────────────

    // 1️⃣ Obtener todos los proyectos manuales activos y no cerrados
    ...
    // 2️⃣ Para cada proyecto, calcular balance real (con última versión de cotización)
    let totalVendido = 0;
    let ingresosRecibidos = 0;
    let totalDescuentos = 0;
    let totalSobrecargas = 0;

    for (const { id } of activeProjects) {
      const balance = await calculateProjectBalance(id);
      // adjustedTotal = totalAmount (última versión cotiz) + sobrecargas - descuentos
      const adjustedTotal = balance.totalAmount + balance.totalSurcharges - balance.totalDiscounts;
      totalVendido      += adjustedTotal;
      ingresosRecibidos += balance.totalPaid;       // solo movementType='payment'
      totalDescuentos   += balance.totalDiscounts;
      totalSobrecargas  += balance.totalSurcharges;
    }

    // 3️⃣ GASTOS — Bug 3 fix: filtrar gastos eliminados (isNull(expenses.deletedAt))
    const expensesResult = await db
      .select({ total: sql<number>`SUM(CAST(expenses.amount AS DECIMAL(12,2)))` })
      .from(expenses)
      .where(isNull(expenses.deletedAt));

    const gastos = parseFloat(expensesResult[0]?.total?.toString() || '0');

    // 4️⃣ Métricas derivadas
...
export async function createAccountingClosure(data: {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    .returning({ id: accountingClosures.id });
