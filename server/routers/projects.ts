...
export const projectMaterialsRouter = router({
    ...
    // Send phase notification by WhatsApp Cloud API
    sendSectionNotification: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        // Validar permisos
        const allowedRoles = ["admin", "super_admin", "comercial", "disenador", "jefe_taller"];
        if (!allowedRoles.includes(ctx.user.role)) {
        ...
        if (!client || !client.whatsappPhone) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "El cliente no tiene numero de WhatsApp registrado" });
        }

        // Construir URL de galeria
        const BASE_URL = process.env.VITE_APP_URL || "https://innovar-cocinas.com";
        const galleryUrl = `${BASE_URL}/gallery?project=${input.projectId}&type=${input.sectionKey}`;

        // Construir mensaje dinamico segun seccion
        let message = "";
        const clientName = client.name || "Cliente";

        switch (input.sectionKey) {
          case "corte":
            message = `Avance de tu proyecto - Corte\n\nHola ${clientName},\n\nTu proyecto ya se encuentra en fase de Corte.\n\nPuedes ver el avance aqui:\n${galleryUrl}\n\nSeguimos trabajando para entregarte la mejor calidad.\n\nINNOVAR Cocinas Integrales`;
            break;
          case "enchape":
            message = `Avance de tu proyecto - Enchape\n\nHola ${clientName},\n\nYa estamos en fase de Enchape.\n\nRevisa el progreso aqui:\n${galleryUrl}\n\nINNOVAR Cocinas Integrales`;
            break;
          case "armado":
            message = `Avance de tu proyecto - Armado\n\nHola ${clientName},\n\nTu proyecto esta en fase de Armado.\n\nPuedes ver imagenes aqui:\n${galleryUrl}\n\nINNOVAR Cocinas Integrales`;
            break;
          case "instalacion":
            message = `Instalacion en proceso\n\nHola ${clientName},\n\nYa estamos realizando la instalacion.\n\nConsulta el avance aqui:\n${galleryUrl}\n\nINNOVAR Cocinas Integrales`;
            break;
          case "entrega":
            message = `Proyecto listo para entrega\n\nHola ${clientName},\n\nTu proyecto esta listo para entrega.\n\nRevisa las imagenes finales aqui:\n${galleryUrl}\n\nINNOVAR Cocinas Integrales`;
            break;
        }
        ...
        if (!response.success) {
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Error al enviar notificacion por WhatsApp" });
        }

        console.log(`[SECTION NOTIFICATION] Notificacion enviada exitosamente. MessageId: ${response.messageId}`);
